...
def scan_line(mode):
	...
	if mode == 0: # normal
		...
		pos_pan += pan_step
		pos_tilt = rospy.get_param("/marathon_params/Tilt_Angle") *-1
		if pos_pan >= pan_max or pos_pan <= pan_min:
			pan_step *= -1 	
...
def line_pos_callback(line_pos_msg):
	...
	line_x = line_pos_msg.data[0]
	line_y = line_pos_msg.data[1]
...
def head_track_line():
	...
	if line_angle != -888:	
		global line_x
		error_x = (float(frame_w)/2.00) - line_x
		...
		rospy.loginfo("Line Angle: %d", line_angle)
		pos_pan, pos_tilt = head_pos.data
		head_pub.publish(head_pos)
